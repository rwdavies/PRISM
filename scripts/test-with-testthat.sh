...
then
    if ! [ -e "PRISM/tests/testthat/test-${what_to_test}.R" ] && ! [ -e "PRISM/tests/testthat/test-${what_to_test}.R" ]
    then
...
R -e 'devtools::document("PRISM"); devtools::load_all("PRISM", quiet = FALSE); devtools::test("PRISM", filter = "'${what_to_test}'", reporter = "summary"); warnings()' 2>&1 | tee ${logfile}

# print(getwd()); library(devtools); load_all("PRISM", quiet = FALSE);

# somehow this gives 0 exit code on parse failure
started_if_1=`cat ${logfile} | grep 'Testing PRISM' | wc -l`
