...
#' @export
getMotifs=function(hot,hotExclude=NULL,eligible=NULL,nCores=1,K=8,hotspotCenterDist=200,filterSequences=FALSE,rmsk=NULL,simpleRepeats=NULL,discovery=TRUE,initializationMethod="advanced",pwmValue=0.9,alpha=0.5,incprob=0.99999,maxits=30,plen=0.05,updatemot=1,updatealpha=1,ourprior=NULL,updateprior=1,bg=-1,plotting="pdf",plotPrefix="getMotifs.plot",verbose=1,scorematset=NULL,dimvec=NULL,pwm=NULL)
{
...
if(verbose>=1) print(paste(" ---- Program Initializing ",date()," -----",sep=""))
if(verbose>=1) print(paste(" ------------------------------------------------------- ",sep=""))
...
if(initializationMethod!="existing")
{
  ...
  ###
  motifEnrichment=getEnrichment(chrList=chrList,hot=hot,hotExclude=hotExclude,nCores=nCores,K=K,hotspotCenterDist=hotspotCenterDist,filterSequences=TRUE,rmsk=rmsk,simpleRepeats=simpleRepeats,verbose=verbose)
  ###
  ### step 0 - do motif enrichment
  ###
  initialData=initializePWM(motifEnrichment=motifEnrichment,K=K,initializationMethod=initializationMethod,verbose=verbose,eligible=eligible)
  pwm=initialData$pwm
...
# if using existing motifs, load sequences only
if(initializationMethod=="existing")
{
  seqsX=getSequences(chrList=chrList,hot=hot,nCores=nCores,filterSequences=filterSequences,rmsk=rmsk,simpleRepeats=simpleRepeats,verbose=verbose)$seqs
...
while(its<=maxits)
{
  ...
  ###
  ### get some background stuff
  ###
  if(verbose>=2)   print(paste("More background stuff - ",date(),sep=""))  
  e=c("A","C","G","T")
  ...
  out=getPredFrac(starts=starts,qvec=qvec,newqvec=newqvec)
